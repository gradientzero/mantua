...
*[Diagram: "Giving Codex a full observability stack in local dev." An app sends logs, metrics, and traces to Vector, which fans out to Victoria Logs / Metrics / Traces, each queried via LogQL, PromQL, or TraceQL. Codex uses these signals to query, correlate, and reason, then implements fixes, restarts the app, re-runs workloads, tests UI journeys, and repeats.]*
...
*[Diagram: "The limits of agent knowledge: What Codex can't see doesn't exist." Codex's knowledge is a bounded bubble; Google Docs, Slack messages and tacit human knowledge sit outside it, and must be encoded into the codebase as markdown to become visible.]*
...
*[Diagram: "Layered domain architecture with explicit cross-cutting boundaries." Within each business domain, code depends forward through Types → Config → Repo → Service → Runtime → UI. Cross-cutting concerns (auth, connectors, telemetry, feature flags) enter through a single explicit Providers interface. Anything else is disallowed and enforced mechanically.]*
